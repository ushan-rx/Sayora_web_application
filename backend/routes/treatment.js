...
router.route("/update/:id").put(async (req, res) => {
  ...
  treatment
    .findOneAndUpdate({ treatmentId: treID }, updatetre)
    .then(() => {
    ...
    .catch((err) => {
      console.log(err);
      res.status(500).send({
        status: "Error with updating the medicine",
        error: err.message,
      });
    });
});
...
router.route("/delete/:id").delete(async (req, res) => {
  ...
  await treatment
    .findOneAndDelete({ treatmentId: treID })
    .then(() => {
    ...
    .catch((err) => {
      ...
      res.status(500).send({
        status: "Error while deleting the medicine",
        error: err.message,
      });
    });
});
...
router.route("/update/:id").put(async (req, res) => {
  ...
  const update = await treatment
    .findOneAndUpdate(treID, updatetre)
    .then(() => {
      res.status(200).send({ status: "Medicine updated" });
    })
    .catch((err) => {
