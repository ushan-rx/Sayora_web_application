...
const createStaff = async (req, res) => {
  try {
    const newStaff = new Staff(req.body); // Use 'Staff' instead of 'staff'
...
const getSingleStaff = async (req, res) => {
  ...
  try {
    const foundStaff = await Staff.findOne({ staffId: staffId }); // Use 'Staff' instead of 'staff'
    if (!foundStaff) {
...
const getAllStaff = async (req, res) => {
  try {
    const allStaff = await Staff.find({ status: true });
