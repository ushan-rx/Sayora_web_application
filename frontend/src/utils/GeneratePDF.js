...
export function generatePDF(title, columns, data, fileName, extraInfo) {
  const doc = new jsPDF({
  ...
  // Add the clinic name to the footer of each page
  ...
  if (!extraInfo) {
    ...
    doc.autoTable({
      ...
      startY: 50, // Start table after logo and title
  ...
  // if extraInfo is provided, add it to the end of the PDF
  if (extraInfo) {
    doc.autoTable({
      body: extraInfo.map((info) => {
    ...
    // show Total at the end of the table - get total from the extraInfo
    const total = extraInfo.find((info) => info.key === "Total").value;
    doc.setFontSize(15);
    doc.text(`Total: Rs.${total}`, 150, doc.autoTable.previous.finalY + 20);
  }
